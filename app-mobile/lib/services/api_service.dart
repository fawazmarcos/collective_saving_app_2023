...
//Récupérations des transactions
  ...
  //Récupérations des demandes retraits
  Future<List<DemandeRetraitModel>> getDemandesRetrait() async {
    QuerySnapshot demandesRetraitsSnapshot = await FirebaseFirestore.instance
    ...
    List<DemandeRetraitModel> demandesRetraitsList = demandesRetraitsSnapshot
        .docs
        .map((document) => DemandeRetraitModel.fromMap(document))
        .toList();

    print(demandesRetraitsList);
    return demandesRetraitsList;
  }
  ...
  Future<List<SouscriptionModel>> getsouscriptions() async {
    ...
    for (final souscriptionDoc in souscriptionsDocs) {
      ...
      */
      souscriptionsList.add(souscription);
    }
